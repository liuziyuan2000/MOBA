...
from models import DGI, LogReg  #DGI的代码
...
def load_data(args):
    ...
    if args.dataset in ['cora', 'citeseer']:
        ...
        adj = nx.adjacency_matrix(nx.from_dict_of_lists(graph)) #graph是一个字典，包含每条边的值
        ...
        indices_norm = torch.from_numpy(np.stack([adj_norm.tocoo().row, adj_norm.tocoo().col], axis=0).astype(float)).long() #有边的行和列都显示出来
        values_norm = torch.from_numpy(adj_norm.tocoo().data.astype(float)).float() #得出所有值的个数
        adj_norm = torch.sparse.FloatTensor(indices_norm, values_norm, torch.Size(adj_norm.shape))
        indices = torch.from_numpy(np.stack([adj.tocoo().row, adj.tocoo().col], axis=0).astype(float)).long() #和上面不同的是这个是原始的邻接矩阵
        #diff = tuple(diff.tolist())
       # diff1 = diff.numpy()
        #diff1= coo_matrix(diff1)
        #indices_1 = torch.from_numpy(np.stack([diff1.row, diff1.col], axis=0).astype(float)).long()
        values = torch.from_numpy(adj.tocoo().data.astype(float)).float() #所以不带对角矩阵，值比values_norm少
    ...
    return adj, diff, adj_norm, features, labels, indices#,indices_1
...
def RECALL_NDCG(estimated_fts, true_fts, topN=10):
    preds = np.argsort(-estimated_fts, axis=1) #对数组每行的列的大小在行中按降序排序，并且写上列的序号，而不是相应的值
    preds = preds[:, :topN]
    ...
    for i in range(preds.shape[0]):
        if len(gt[i]) != 0:
            if np.sum(estimated_fts[i, :]) != 0:
                recall = len(set(preds[i, :]) & set(gt[i])) * 1.0 / len(set(gt[i])) #求两个集合中相同数的个数
                recall_list.append(recall)

                intersec = np.array(list(set(preds[i, :]) & set(gt[i]))) #注意化为数组前集合变为list
                if len(intersec) > 0:
                    dcg = [np.where(preds[i, :] == ele)[0] for ele in intersec] #相同的相应位置
                    dcg = np.sum([1.0 / (np.log2(x + 1 + 1)) for x in dcg])
                    idcg = np.sum([1.0 / (np.log2(x + 1 + 1)) for x in range(len(gt[i]))])#推荐的越多越好，且越小越好
                    ndcg = dcg * 1.0 / idcg
                else:
                    ndcg = 0.0
...
def class_eva(train_fts, train_lbls, test_fts, test_lbls):
    ...
    test_non_featured_idx = np.where(test_fts.sum(1) == 0)[0] #结果为0
    ...
    fts_dim = train_fts.shape[1] #1433
    hid_dim = 64#64
    n_class = int(max(max(train_lbls), max(test_lbls)) + 1)
    is_cuda = torch.cuda.is_available()
    #model = DGI(fts_dim, hid_dim, n_class)
    ...
    for i in range(1000):
        ...
        random_preds = np.random.choice(n_class, len(test_non_featured_idx)) #结果为0

        preds = np.concatenate((featured_preds, random_preds)) #所以维度和featured_preds一样，都为271
...
def data_split(args, adj):
    ...
    train_fts_idx = torch.from_numpy(shuffled_nodes[:int(args.train_fts_ratio * adj.shape[0])]).long() #取了s_n的0到2708*0.4(ratio)个数，一共1083个
    vali_fts_idx = torch.from_numpy(
        shuffled_nodes[
        int(args.train_fts_ratio * adj.shape[0]):int((args.train_fts_ratio + 0.1) * adj.shape[0])]).long() #取了s_n的2708*0.4(ratio)到2708*0.5(ratio)个数
    test_fts_idx = torch.from_numpy(shuffled_nodes[int((args.train_fts_ratio + 0.1) * adj.shape[0]):]).long() #1354
    vali_test_fts_idx = torch.from_numpy(shuffled_nodes[int(args.train_fts_ratio * adj.shape[0]):]).long() #1625
...
def data_split(args, adj):
    ...
    vali_fts_idx = torch.from_numpy(
        shuffled_nodes[
        int(0.4* adj.shape[0]):int((0.4 + 0.1) * adj.shape[0])]).long()
    #test_fts_idx = torch.from_numpy(shuffled_nodes[int((args.train_fts_ratio + 0.1) * adj.shape[0]):]).long() #1354
    test_fts_idx = torch.from_numpy(shuffled_nodes[int((0.4+ 0.1) * adj.shape[0]):]).long()
    #vali_test_fts_idx = torch.from_numpy(shuffled_nodes[int(args.train_fts_ratio * adj.shape[0]):]).long() #1625
    vali_test_fts_idx = torch.from_numpy(shuffled_nodes[int(0.4 * adj.shape[0]):]).long()
    print("Dataset loading done!")
    return train_fts_idx, vali_fts_idx, test_fts_idx, vali_test_fts_idx
'''
def loss_weight(args, true_features, train_fts_idx):
    if args.dataset in ['cora', 'citeseer', 'amac', 'amap', 'steam']:
        ...
        pos_weight = torch.sum(true_features[train_fts_idx] == 0.0).item() / (
            torch.sum(true_features[train_fts_idx] != 0.0).item()) #要计算的是训练索引共1083个点中零点和非零点的比值
    else:
        fts_loss_func = None
        pos_weight = None
        print("Error!")
...
def input_matrix(args, adj, norm_adj, true_features):
    indices = torch.from_numpy(np.stack([np.arange(adj.shape[0]), np.arange(adj.shape[0])], axis=0)).long() #[2,2708]
    values = torch.from_numpy(np.ones(indices.shape[1])).float()#tensor(2708,),一共2708个1
...
def mask_edge(edge_index, p=0.7): #通过概率随机去除百分之20的边，并用bool值表示
    if p < 0. or p > 1.:
        raise ValueError(f'Mask probability has to be between 0 and 1 '
                         f'(got {p}')
    e_ids = torch.arange(edge_index.size(1), dtype=torch.long, device=edge_index.device)
    mask = torch.full_like(e_ids, p, dtype=torch.float32) #创建一个与 e_ids 张量形状相同的张量，其所有元素的值都设置为 p。这里 p 是一个概率值，通常在 0 到 1 之间。
    mask_ = torch.bernoulli(mask).to(torch.bool) #对 mask 中的每个元素进行伯努利采样，将其转换为二值（0或1），概率为相应位置上的值。因为 mask 中的元素都是概率值，所以这里进行伯努利采样就相当于按照概率 p 生成二值序列。
    # print(mask.size())
...
def test_X(gene_fts, labels_of_gene):#1433

    print('begining test_X......')

    gene_data = np.concatenate((gene_fts, np.reshape(labels_of_gene, newshape=[-1, 1])), axis=1) #加了一列，np.reshape的作用是将 labels_of_gene 数组转换为一个列向量，其中 -1 表示该维度的大小由数组本身和其他维度的大小来确定，而 1 表示这一维度的大小为1
    final_list = []
    for i in range(10):
        gene_data = shuffle(gene_data, random_state=72) #应该默认打乱行
        KF = KFold(n_splits=5) #KFold 是交叉验证中常用的一种分割数据集的方法。它将数据集分成 k 个折叠（folds），每个折叠都用作一次验证，其余的 k-1 个折叠用作训练。
        ...
        for train_idx, test_idx in split_data:
            ...
            train_data = train_data[train_featured_idx] #等于还是631的train_data
...
def test_AX(gene_data, labels_of_gene, adj):#adj[1354,1354]
    ...
    n_nodes = adj.shape[0]
    indices = np.where(adj != 0) #生成了（两个tuple类型）看看怎么生成索引的
    rows = indices[0]
    cols = indices[1]
    adj = sp.coo_matrix((np.ones(shape=len(rows)), (rows, cols)), shape=[n_nodes, n_nodes]) #如何由索引造含边的图
    adj = normalize_adj(adj + sp.eye(adj.shape[0]))
    indices = torch.LongTensor(np.int64(np.stack([adj.tocoo().row, adj.tocoo().col], axis=0))) #加了自环后由scr稀疏矩阵adj重新生成索引
    values = torch.FloatTensor(adj.tocoo().data)
    adj = torch.sparse.FloatTensor(indices, values, torch.Size(adj.shape)) #生成稀疏张量
    labels_of_gene = torch.LongTensor(labels_of_gene)
    n_class = max(labels_of_gene).item() + 1
    #features = torch.FloatTensor(gene_data) #放在这里有问题，移到722行了
    ...
    for i in range(10):
        ...
        for train_idx, test_idx in split_data:
            ...
            featured_train_idx = train_idx[(train_fts.sum(1)!= 0).nonzero().reshape([-1])] #全部行
            featured_test_idx = test_idx[(test_fts.sum(1) != 0).nonzero().reshape([-1])] #全部行
            non_featured_test_idx = test_idx[(test_fts.sum(1) == 0).nonzero().reshape([-1])] #0行
            featured_train_lbls = labels_of_gene[featured_train_idx]
            featured_test_lbls = labels_of_gene[featured_test_idx]
            non_featured_test_lbls = labels_of_gene[non_featured_test_idx] #以上都是张量
            featured_test_lbls_arr = featured_test_lbls.numpy() #数组
            non_featured_test_lbls_arr = non_featured_test_lbls.numpy()
            model = GCN_eva(nfeat=features.shape[1], nhid=64, nclass=n_class, dropout=0.1, input_fts_sparse=False) #用来两层GCN加一个全连接层，分别是（1433，64），（64，64），（64，7）
            if is_cuda:
                model.cuda()
                adj = adj.cuda()
            ...
            for epoch in range(1000):
                ...
                random_preds = np.random.choice(np.arange(n_class), len(non_featured_test_idx)) #为0，没有用
...
def loss_fn(x, y):  # [45914,1024]
    x = F.normalize(x, dim=-1,
                    p=2)  # p=2: 这是指定归一化的范数。在这里，p=2 表示使用 L2 范数进行归一化，即将输入张量的每个向量除以其 L2 范数，使得每个向量的长度都归一化为 1。
    y = F.normalize(y, dim=-1, p=2)  # 归一化后维度都没变

    return (2 - 2* (x * y).sum(dim=-1))#.pow(2)#torch.exp

def loss_fn_n(x, y):  # [45914,1024]
    x = F.normalize(x, dim=-1,
                    p=2)  # p=2: 这是指定归一化的范数。在这里，p=2 表示使用 L2 范数进行归一化，即将输入张量的每个向量除以其 L2 范数，使得每个向量的长度都归一化为 1。
    y = F.normalize(y, dim=-1, p=2)  # 归一化后维度都没变
...
def create_sparse(I):
        #I.reshape(-1).type: 'torch.cuda.LongTensor'
    similar = I.reshape(-1).tolist()#将多维张量转换为列表 reshape(-1) 是一个在 NumPy 或类似库中常见的操作，用于将数组或张量重新整形为一维数组。具体来说，reshape(-1) 将数组或张量转换为一个一维的向量，其中的元素顺序保持不变。
    index = np.repeat(range(I.shape[0]), I.shape[1])#根据列数重复写出每行[0000111122223333...46804 46804 46804]
    assert len(similar) == len(index)
    indices = torch.tensor([index, similar]).cuda() #[2,46804] 将ndarray和list转为两个tensor
    result = torch.sparse_coo_tensor(indices, torch.ones_like(I.reshape(-1)), [I.shape[0], I.shape[0]]) #torch.sparse_coo_tensor 是 PyTorch 中用于创建稀疏张量的函数之一。COO 表示 Coordinate (坐标) 格式，在这种格式下，稀疏矩阵的非零元素由它们的坐标和值来表示。
    return result
